...
fn main() -> Result<(), Error> {
    ...
    let properties = get_mlir_properties(&llvm_build_dir)?;
    ...
    let ll_bindings = bindings
        .header("wrapper.h")
        .parse_callbacks(Box::new(bindgen::CargoCallbacks::new()))
        .generate()?;

    let out_path = PathBuf::from(env::var("OUT_DIR")?);
    ll_bindings.write_to_file(out_path.join("low-level.rs"))?;
    let hl_bindings = hl::gen(&std::fs::read_to_string(out_path.join("low-level.rs"))?)?;
    let mut hl_bindings_file = std::fs::File::create(out_path.join("high-level.rs"))?;
    write!(hl_bindings_file, "{hl_bindings}")?;
    Ok(())
}
